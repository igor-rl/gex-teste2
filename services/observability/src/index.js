...
async function getQueueMetrics(name, url) {
  ...
  try {
    ...
    return {
      ...
      messages_inflight:   parseInt(attrs.ApproximateNumberOfMessagesNotVisible || '0', 10),
      messages_delayed:    parseInt(attrs.ApproximateNumberOfMessagesDelayed  || '0', 10),
    }
  } catch (err) {
...
async function start() {
  ...
  app.get('/metrics/queues', async () => {
    const metrics = await Promise.all(
      Object.entries(QUEUE_URLS).map(([name, url]) => getQueueMetrics(name, url))
    )
    return { queues: metrics.filter(Boolean), ts: new Date().toISOString() }
  })
  ...
  app.get('/metrics/summary', async () => {
    ...
    for (const r of rows) {
      summary[r.status] = {
        total:          +r.total,
        invalid_email:  +r.invalid_email,
        invalid_phone:  +r.invalid_phone,
  ...
  // ─── Latência média (tempo venda → processamento) ─────────────
  ...
  // ─── Reconciliação: DB vs esperado ────────────────────────────
  app.get('/metrics/reconciliation', async () => {
    const [queueMetrics, { rows: dbRows }] = await Promise.all([
    ...
    const db_total = +dbRows[0].total
    return {
      db:             { total: db_total, ...dbRows[0] },
      queue_inflight: queueMetrics?.messages_inflight || 0,
      queue_waiting:  queueMetrics?.messages_available || 0,
  ...
  app.get('/metrics/recent-errors', async () => {
    const { rows } = await db.query(`
      SELECT order_id, email, product_name, error_message, attempt_count, processed_at
      FROM lead_control
      WHERE status = 'erro'
  ...
  // ─── Audit trail por order_id ─────────────────────────────────
  app.get('/audit/:orderId', async (req) => {
    const { rows } = await db.query(`
      SELECT event_type, service, payload, error_detail, duration_ms, ts
  ...
  // ─── DLQ: mensagens mortas (falhou depois de tudo) ────────────
  app.get('/metrics/dlq', async () => {
    const metrics = await getQueueMetrics('dlq', QUEUE_URLS.dlq)
    return {
      dlq: metrics,
      alert: (metrics?.messages_available || 0) > 0,
      ts: new Date().toISOString(),
    }
  })

  // ─── Pipeline health: último processamento ────────────────────
  app.get('/metrics/pipeline-health', async () => {
    const { rows } = await db.query(`
      SELECT
        MAX(processed_at) AS last_processed,
        EXTRACT(EPOCH FROM (NOW() - MAX(processed_at))) / 60 AS minutes_since_last,
        COUNT(*) FILTER (WHERE processed_at > NOW() - INTERVAL '1 hour') AS last_hour_count
      FROM lead_control
    `)
    const minutesSince = +rows[0].minutes_since_last
    const stalled      = minutesSince > 30

    return {
      last_processed:      rows[0].last_processed,
      minutes_since_last:  Math.round(minutesSince),
      last_hour_count:     +rows[0].last_hour_count,
